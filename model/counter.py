...
class NgramModelVocabulary(Counter):
    ...
    - When checking membership and calculating its size, filters items by
      comparing their counts to a cutoff value.
    ...
    def __len__(self):
        """This should reflect a) filtering items by count, b) accounting for
        unknowns. The first is achieved by relying on the membership check
        implementation. The second is achieved by adding 1 to vocabulary size.
        """
...
class NgramCounter(object):
    ...
    def _set_up_vocabulary(self, vocabulary, unk_cutoff):
        ...
        # copy needed to prevent state sharing
    ...
    def train_counts(self, training_text):
        ...
        if len(self.vocabulary) <= 1:
            raise EmptyVocabularyError("Cannot start counting ngrams until "
                                       "vocabulary contains more than one \
                                       item.")
        ...
        for sent in training_text:
            ...
            for ngram in self.to_ngrams(checked_sent):
                ...
                for trunc_index, ngram_order in self._enumerate_ngram_orders():
                    ...
                    # note that above line doesn't affect context on first
                    # iteration
